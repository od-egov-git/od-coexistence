			...
			<tbody>
				<c:choose>
					<c:when test="${assetList!=null && assetList.size() > 0}">
						<c:forEach items="${assetList}" var="disposal" varStatus="item">
							...
							<tr id="assetView">
								...
								--%>
								<td><a href="sale/view/${disposal.id}" target="popup"
									onclick="window.open('${contextPath}/sale/view/${disposal.id}','popup','width=700,height=600'); return false;">
										${item.index + 1} </a></td>
								<td><a href="sale/view/${disposal.id}" target="popup"
									onclick="window.open('${contextPath}/sale/view/${disposal.id}','popup','width=700,height=600'); return false;">
										${disposal.asset.code } </a></td>
								<td><a href="sale/view/${disposal.id}" target="popup"
									onclick="window.open('${contextPath}/sale/view/${disposal.id}','popup','width=700,height=600'); return false;">
										${disposal.asset.assetHeader.assetName }</a></td>
								<td><a href="sale/view/${disposal.id}" target="popup"
									onclick="window.open('${contextPath}/sale/view/${disposal.id}','popup','width=700,height=600'); return false;">
								...
								<%-- <c:if test="${isReference}">
									<td><input type="button" class="btn btn-default"
										value="Select"
										onclick="selectAssetRef('${disposal.asset.code}','${disposal.asset.assetHeader.assetName }')" />
									</td>
								</c:if> --%>
							</tr>
						</c:forEach>
