...
<div class="panel panel-primary" data-collapsed="0" style=" scrollable:true;">
    ...
    <c:if test="${disposal.documents != null &&  !disposal.documents.isEmpty()}">
    <table>
        <c:forEach items="${disposal.documents }" var="documentDetials">
        <tr>    
        	<td class="padding-10"><a href="/services/asset/sale/downloadSaleDisposalDoc?disposalId=${disposal.id }&fileStoreId=${documentDetials.fileStore.fileStoreId }">${documentDetials.fileStore.fileName }</a></td>
        	<td class="padding-10"><span><input type="button" id="remove"
				style="background: #265988" value="Remove"
				onclick="deletedoc(${disposal.id },${documentDetials.id });"></span></td>
		</tr><br>	
        </c:forEach>
        </table>
    </c:if>
    ...
    <input type="hidden" value="${fn:length(disposal.documents)}" id="documentsSize">
