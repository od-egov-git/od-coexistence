...
public interface DisposalRepository extends JpaRepository<Disposal, Long> {
	...
	@Query(value = "From AssetMaster am left join DepreciationInputs di on di.assetId=am.id where am.code=:code or am.assetHeader.assetName=:name or am.assetHeader.assetCategory.id=:category or am.assetHeader.department=:department  or am.assetStatus.id=:status and am.assetStatus.code!='CAPITALIZED'")
	public List<AssetMaster> getAssetMasterDetails(@Param("code") String code, @Param("name") String name, @Param("category") Long category, @Param("department") String department, @Param("status") Long status);

	@Query(value = "From AssetMaster am where am.code=:code or am.assetHeader.assetName=:name or am.assetHeader.assetCategory.id=:category   or am.assetStatus.id=:status")
	public List<AssetMaster> getAssetMasterDetails(@Param("code") String code, @Param("name") String name, @Param("category") Long category, @Param("status") Long status);
