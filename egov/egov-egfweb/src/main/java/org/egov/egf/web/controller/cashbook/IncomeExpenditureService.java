 ...
 AppConfigValueService appConfigValuesService;
   ...
   // Date fromDate;
    ...
    public void populateIEStatement(Statement ie) {
        ...
        System.out.println("### minorCodeLength in expenditure ::"+minorCodeLength);
    	}catch(Exception ex) {
        ...
        coaType.add('I');
        coaType.add('E');
        Date fromDate = ie.getFromDate();
        Date toDate = ie.getToDate();
		/*
		 * Date fromDate = getFromDate(ie); Date toDate = getToDate(ie);
		 */
    ...
    private List<StatementResultObject> getBudgetForMajorCodes(final Statement incomeExpenditureStatement) {
        ...
        queryStr.append("where ((bg.maxcode<=coa.id and bg.mincode>=coa.id) or bg.majorcode=coa.id ) and bd.budgetgroup= bg.id "
                +
                " and bd.budget=b.id and  bd.state_id=wfs.id  and wfs.value='END'  and b.isbere=:isBeRe   ");
        ...
        if(incomeExpenditureStatement.getFinancialYear()!= null && incomeExpenditureStatement.getFinancialYear().getId() != null
        		&& incomeExpenditureStatement.getFinancialYear().getId() != 0) {
        	queryStr.append(" and b.financialyearid= "+incomeExpenditureStatement.getFinancialYear().getId());
        	}
        ...
        budgteQuery.setString("isBeRe", "RE");
        }else {
    ...
    private List<StatementResultObject> getBudgetReappMinorCodes(final Statement incomeExpenditureStatement) {
        ...
        queryStr.append(" from egf_budgetdetail bd , egf_budgetgroup bg,egf_budget b, chartofaccounts coa,eg_wf_states wfs,egf_budget_reappropriation bdr where ((bg.maxcode<=coa.id and bg.mincode>=coa.id) or bg.majorcode=coa.id ) and bd.budgetgroup= bg.id "
                +
                "  and bdr.budgetdetail=bd.id and bd.budget=b.id and bdr.state_id=wfs.id  and wfs.value='END' and b.isbere=:isBeRe  ");
        ...
        if(incomeExpenditureStatement.isRestData()) {
            ...
            budgteReappQuery.setString("isBeRe", "RE");
        }else {
