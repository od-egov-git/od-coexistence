...
public class PreApprovedVoucherAction extends GenericWorkFlowAction {
	...
	private VoucherTypeBean voucherTypeBean = new VoucherTypeBean();
	...
	public String removeVoucher() {
		...
		if (instrumentVoucherId != null && voucherid != null) {
			...
			if (instrumentHeaderDtl != null) {
				...
				// balanceAmount=
				// bigDecimalinstrumrntamount.subtract(instrumentHeaderq.getInstrumentAmount());

				query = persistenceService.getSession()
						.createSQLQuery("update egf_instrumentheader set instrumentamount=" + balanceAmount
								+ " where id=" + instrumentHeaderq.getId());
				int amountupdate = query.executeUpdate();

				// delete from egf_instrumentvoucher
				query = persistenceService.getSession()
						.createSQLQuery("delete from egf_instrumentvoucher where voucherheaderid= " + voucherid
								+ " and id=" + instrumentVoucherId);
	...
	public String loadvoucher() {
		...
		if (null != voucherHeader.getType()) {
			if (voucherHeader.getType().equalsIgnoreCase("Receipt")) {
				...
				egBillregister = voucherService.getegBillRegister(voucherHeader);
				if (null != egBillregister) {
					final EgBillregistermis egBillregistermis = egBillregister.getEgBillregistermis();

					if (null != egBillregister.getBilldate())
						voucherTypeBean.setBillDate(egBillregister.getBilldate());
					else
						voucherTypeBean.setBillDate(voucherHeader.getVoucherDate());
					if (null != egBillregister.getBillnumber()
							&& StringUtils.isNotEmpty(egBillregister.getBillnumber()))
						if (!voucherTypeBean.getBillNum().equalsIgnoreCase(egBillregister.getBillnumber())) {
							if (!voucherService.isBillNumUnique(egBillregister.getBillnumber()))
							...
							voucherTypeBean.setBillNum(egBillregister.getBillnumber());
						}
					voucherTypeBean.setPartyName(egBillregistermis.getPayto());
					voucherTypeBean.setPartyBillNum(egBillregistermis.getPartyBillNumber());
					voucherTypeBean.setPartyBillDate(egBillregistermis.getPartyBillDate());
		...
		if (voucherHeader.getState() != null && voucherHeader.getState().getValue().contains("Rejected"))
			if (voucherHeader.getModuleId() == null) {
				...
				// added by deep for type receipt
		...
		if (!ismodifyJv)
			if ("Y".equals(pjv_wc_enabled))
				...
				if (voucherHeader.getState().getValue() != null
						&& voucherHeader.getState().getValue().equalsIgnoreCase("SaveAsDraft")) {
					result = VOUCHEREDIT;
					ismodifyJv = true;
		...
		voucherHeader = (CVoucherHeader) getPersistenceService().find(VOUCHERQUERY,
				Long.valueOf(parameters.get(VHID)[0]));
		List<DocumentUploads> voucherDocList = voucherService.findByObjectIdAndObjectType(voucherHeader.getId(),
	...
	@SkipValidation
	@Action(value = "/voucher/preApprovedVoucher-downloadVoucherDoc", results = {
			@Result(name = "success", type = "stream", params = { "contentType", "${mimeType}", "inputName",
					"inputStream", "bufferSize", "4096", "contentDisposition", "filename=\"${fileName}\"" }) })
	public String downloadVoucherDoc() throws ApplicationException, IOException {
		...
		List<DocumentUploads> voucherDocList = voucherService.findByObjectIdAndObjectType(
				Long.valueOf(parameters.get("voucherHeaderId")[0]), CommonConstants.JOURNAL_VOUCHER_OBJECT);
		voucherHeader.setDocumentDetail(voucherDocList);
	...
	/*
	 * serResponse.setContentType(mimeType); serResponse.setContentLength((int)
	 * downloadFile.length());
	 * 
	 * // set headers for the response final String headerKey =
	 * "Content-Disposition"; final String headerValue =
	 * String.format("attachment; filename=\"%s\"", fileName);
	 * serResponse.setHeader(headerKey, headerValue);
	 * 
	 * // get output stream of the response final OutputStream outStream =
	 * serResponse.getOutputStream();
	 * 
	 * final byte[] buffer = new byte[BUFFER_SIZE]; int bytesRead = -1;
	 * 
	...
	public String update() throws ValidationException {
		...
		try {
			...
			// microserviceUtils.getEmployeeByPositionId(voucherHeader.getState().getOwnerPosition());
			...
			if (FinancialConstants.BUTTONREJECT.equalsIgnoreCase(workflowBean.getWorkFlowAction()))
				addActionMessage(
						getText("pjv.voucher.rejected", new String[] { rejEmpName != null ? rejEmpName : "" }));
			if (FinancialConstants.BUTTONFORWARD.equalsIgnoreCase(workflowBean.getWorkFlowAction()))
				addActionMessage(getText("pjv.voucher.approved", new String[] { empName != null ? empName : "" }));
		...
		if (parameters.get(VHID)[0].equals("")) {
			...
			if (budgetcheck) {
				...
				voucherHeader.transition().start().withOwner(getPosition())
						.withOwnerName((getPosition().getId() != null && getPosition().getId() > 0L)
								? getEmployeeName(getPosition().getId())
								: "");
	 ...
	 * List<CChartOfAccounts> glcodeList = null; final Query glcodeListQuery =
	 ...
	 * ); glcodeListQuery.setParameterList("IDS", glcodeIdList); glcodeList =
	 * glcodeListQuery.list(); if (glcodeIdList.isEmpty())
	 * dropdownData.put("glcodeList", Collections.EMPTY_LIST); else
