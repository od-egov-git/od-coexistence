...
function addFileInputField() {
	...
	var tempTrNo = trNo - 1; 
	//var curFieldValue = $("#supportingDocuments[" + tempTrNo+"]").val();
	...
	inputFile.setAttribute("name", "supportingDocuments");
	inputFile.setAttribute("id", "contingentBill_supportingDocuments");
...
function addMore()
{
	var fileId=document.getElementById("hiddenCounter").value;
	if(fileId<=9)
		{
		var div=document.getElementById("f"+fileId);
		if (div.style.display === "none") {
			div.style.display = "block";
		  } else {
			  div.style.display = "none";
		  }
		document.getElementById("hiddenCounter").value=parseInt(fileId)+1;
		}
	else
		{
		bootbox.alert("Only 9 documents are allowed for upload");
		}
}
