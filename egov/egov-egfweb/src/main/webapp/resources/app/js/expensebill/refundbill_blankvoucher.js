...
/*function changeGlCodeBlank(ele){
	var eleId = ele.id;
	var arr = eleId.split(".");
	var glcodeid = document.getElementById(arr[0]+".glcodeid").value;
	var glcode = document.getElementById(arr[0]+".glcode").value;
    var k ='<option value="'+glcodeid+'">'+glcode+'</option>';
		  $('#glcodeid').append(k);  
	alert(arr[0]);
 ...
 var dt=$('.creditGlcode').typeahead({
 	hint : true,
	  ...
	  var originalglcodeid = data.id;
	   ...
	   var flag = false;
	   $('#tblcreditdetails  > tbody > tr:visible[id="creditdetailsrow"]').each(function(index) {
		   var glcodeid =document.getElementById('billDetails['+index+'].glcodeid').value;
		   //var detailtypeid = document.getElementById('billDetails['+index+'].detailTypeId').value;
		   var detailkeyid = document.getElementById('billDetails['+index+'].detailKeyId').value;
			if( glcodeid!= "" && originalglcodeid == glcodeid &&  originaldetailtypeid == detailtypeid && originaldetailkeyid == detailkeyid) {
				flag = true;
			}
	   });
	   if(data.issubledger && originaldetailtypeid!='' && originaldetailkeyid=='')
	    {
		   bootbox.alert($.i18n.prop('msg.please.enter',subLedgerDisplayName),function() {
			   var index= dt.length - 1;
				document.getElementById('billDetails['+index+'].creditGlcode').value = "";
			});
	    }else if(flag){
			bootbox.alert($.i18n.prop('msg.credit.code.already.added'), function() {
				var index= dt.length - 1;
