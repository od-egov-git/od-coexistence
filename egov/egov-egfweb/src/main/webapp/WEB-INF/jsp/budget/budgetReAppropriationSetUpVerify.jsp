...
function createAmountFieldFormatter(values,prefix,suffix){
	...
	var makeBudgetDetailTable = function() {
		var budgetDetailColumns = [ 
			...
			<s:if test="%{shouldShowField('function')}">				
				{key:"budgetDetail.function.id",label:'Function',width:90,formatter:createDropdownFormatter(BUDGETDETAILLIST),  dropdownOptions:functionOptions} ,
			</s:if>
			<s:if test="%{shouldShowField('scheme')}">				
			...
			<s:if test="%{shouldShowField('fund')}">				
				{key:"budgetDetail.fund.id",label:'Fund',width:90,formatter:createDropdownFormatter(BUDGETDETAILLIST),  dropdownOptions:fundOptions} ,
			</s:if>
			<s:if test="%{shouldShowField('boundary')}">				
