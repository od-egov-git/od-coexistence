...
<script type="text/javascript">
	...
	function validateFormAndSubmit() {
		...
		if (accountType == 0) {
			...
			dom.get("errorDiv").style.display = '';
			dom.get("errorDiv").innerHTML = 'Please enter Cheque/DD/PEX Number.';
			return false;
		} else if (accountDDDate == "") {
			dom.get("errorDiv").style.display = '';
			dom.get("errorDiv").innerHTML = 'Please enter Cheque/DD/PEX Date.';
			return false;
		} else {
			dom.get("errorDiv").style.display = 'none';
			dom.get("errorDiv").innerHTML = '';
		}
		document.dishonorForm.action = '/EGF/payment/stopPayment-list.action';
	...
	function processDishonorCheque() {
		...
		for (i = 0; i < len; i++) {
			...
			if (refFlag == true && (referenceNum == null || referenceNum == "")) {
				bootbox.alert("Please enter	Reference No for selected Cheque/DD/PEX Number.");
				undoLoadingMask();
				return false;
			}
			...
			bootbox.confirm("Are you sure you want to process ?", function(result) {
				  if(result)
					  ...
					  console.log("referenceNum" + referenceNum);  
						  document.dishonorForm.action = '/EGF/payments/stopPayment-process.action?instHeaderIds='
