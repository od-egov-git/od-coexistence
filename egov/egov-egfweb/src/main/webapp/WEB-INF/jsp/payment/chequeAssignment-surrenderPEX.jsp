 <%--
  ~    eGov  SCity eGovernance suite aims to improve the internal efficiency,transparency,
...
<title><s:text name="surrender.pex" /></title>
</head>
<body>
	...
	<s:form action="chequeAssignment" theme="simple">
		...
		<jsp:include page="../budget/budgetHeader.jsp">
			<jsp:param name="heading" value="Surrender PEX" />
		</jsp:include>
				...
				<table align="center" width="100%" cellpadding="0" cellspacing="0">
					<tr>
						...
						<th class="bluebgheadtdnew"><s:text
								name="chq.assignment.pex.transacton.no" /></th>
						<th class="bluebgheadtdnew"><s:text
								name="chq.assignment.pex.amount" /></th>
						<th class="bluebgheadtdnew"><s:text
								name="chq.assignment.pex.transaction.date" /></th>
						<th class="bluebgheadtdnew"><s:text
	...
	var mode="view";
 	...
 	function surrenderChq(){
 		...
 		document.chequeAssignment.action = '/services/EGF/payment/chequeAssignment-save.action?containsPEX=true';
		document.chequeAssignment.submit();
		return true;
 	}
  	function Reassign()
 	{
		resetSelectedRowsId();
	 	...
	 	disableAll();
	 	document.chequeAssignment.action = '/services/EGF/payment/chequeAssignment-save.action?containsPEX='+document.getElementById('containsPEX').value;
		document.chequeAssignment.submit();
				...
				for(var i=0;i<document.forms[0].length;i++)
					{
						if(document.forms[0].elements[i].name != 'bankaccount' && document.forms[0].elements[i].name != 'bank_branch'
							...
							&& document.forms[0].elements[i].name != 'containsPEX' && document.forms[0].elements[i].name != 'voucherNumber'
